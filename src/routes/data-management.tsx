...
function QueryBuilderPage() {
  ...
  if (!activeQuery) {
    return (
      <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] p-6 text-sm text-[var(--sea-ink-soft)]">
        No queries available. Create one to continue.
      </div>
    )
  }
  ...
  const previewRows = useMemo(
    () => (activeQuery ? runQuery(activeQuery.id).slice(0, 50) : []),
        ...
        {!activeTable ? (
          ...
          <section className="space-y-4 rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] p-4">
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateQuery(activeQuery.id, { name: e.target.value })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateQuery(activeQuery.id, { tableId: e.target.value })}
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateQuery(activeQuery.id, { selectedColumns: next })
                        }}
                      />
                      <span>{col.name}</span>
                    </label>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateQuery(activeQuery.id, { groupBy: next })
                        }}
                      />
                      <span>{col}</span>
                    </label>
            ...
            <div className="flex justify-end">
              <Button type="button" variant="destructive" size="sm" onClick={() => removeQuery(activeQuery.id)}>
                Delete query
              </Button>
            </div>
