...
export interface QueryComputedField {
  id: string
  alias: string
  /** Expression using result column names and + - * / ( ) */
  expression: string
}
...
function tokenize(expression: string, row: DataRow): Tok[] {
  ...
  while ((m = re.exec(expression)) !== null) {
    ...
    if (m[1]) {
      ...
      tokens.push({ kind: 'op', value: m[3] as Tok extends { kind: 'op'; value: infer V } ? V : never })
    }
  }
...
/**
 * Evaluate a safe arithmetic expression against a result row.
 * Only identifiers matching row keys, numbers, and + - * / ( ) are allowed.
 */
